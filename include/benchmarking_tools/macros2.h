...
#define RATE_LOG(name, level, ws, name_prefix)\
    ...
    if (dup_map.find(name) == dup_map.end()){\
        benchmarking_tools::Tracker<benchmarking_tools::Rate> tracker(name, ws, level, 0, name_prefix);\
...
#define RATE_LOG_THROTTLED(name, period, level, ws, name_prefix)\
    static std::map<std::string, benchmarking_tools::Tracker<benchmarking_tools::Rate>> dup_map;\
    if (dup_map.find(name) == dup_map.end()){\
        benchmarking_tools::Tracker<benchmarking_tools::Rate> tracker(name, ws, level, period, name_prefix);\
        dup_map.insert(std::pair<std::string, benchmarking_tools::Tracker<benchmarking_tools::Rate>>(name, tracker));\
    }\
...
#define DELAY_LOG(name, level, ws, header, name_prefix)\
    ...
    if (dup_map_delay.find(name) == dup_map_delay.end()){\
        benchmarking_tools::Tracker<benchmarking_tools::Delay> tracker(name, ws, level, 0, name_prefix);\
...
#define DELAY_LOG_THROTTLED(name, period, level, ws, header, name_prefix)\
    static std::map<std::string, benchmarking_tools::Tracker<benchmarking_tools::Delay>> dup_map_delay;\
    if (dup_map_delay.find(name) == dup_map_delay.end()){\
        benchmarking_tools::Tracker<benchmarking_tools::Delay> tracker(name, ws, level, period, name_prefix);\
        dup_map_delay.insert(std::pair<std::string, benchmarking_tools::Tracker<benchmarking_tools::Delay>>(name, tracker));\
    }\
    static benchmarking_tools::Delay delay_obj;\
    dup_map_delay.find(name)->second.addValue(delay_obj.addValue(header));\
...
#define DURATION_LOG(name, level, ws, name_prefix)\
    ...
    if (dup_map_duration.find(name) == dup_map_duration.end()){\
        benchmarking_tools::Tracker<benchmarking_tools::TimingDuration> tracker(name, ws, level, 0, name_prefix);\
...
#define DURATION_LOG_THROTTLED(name, period, level, ws, name_prefix)\
    static std::map<std::string, benchmarking_tools::Tracker<benchmarking_tools::TimingDuration>> dup_map_duration;\
    if (dup_map_duration.find(name) == dup_map_duration.end()){\
        benchmarking_tools::Tracker<benchmarking_tools::TimingDuration> tracker(name, ws, level, period, name_prefix);\
        dup_map_duration.insert(std::pair<std::string, benchmarking_tools::Tracker<benchmarking_tools::TimingDuration>>(name, tracker));\
    }\
